...
**各パネルから何が読み取れるか(計器の読み方)は [docs/indicators.md](./docs/indicators.md) を参照。**
...
make dev     # ダッシュボード開発サーバ (http://localhost:37414)
```

Web のポートはローカル(`make dev`)・Docker とも **37414** に統一してある(既定 3000 からの変更)。環境変数 `MACRO_WEB_PORT` で上書きできる:

```sh
MACRO_WEB_PORT=48000 make dev   # 例: 48000 で起動
```
...
ローカルに uv / Node を入れずに、Docker だけで取得とダッシュボード(web)を動かせる。常駐サービスは2つ:

- **web** — ダッシュボード(http://localhost:37414。ポートは `MACRO_WEB_PORT` で変更可)
- **scheduler** — 毎日 `FETCH_AT`(既定 06:30 JST)に全系列を取得する常駐スケジューラ。**cron の設定は不要**

手動でのワンショット取得用に **fetcher** サービス(`docker compose run --rm fetcher`。`profiles: ["fetch"]` により `up` の対象外)も残してある。scheduler と fetcher は同一イメージを共有する(command 差し替えのみ)。

> **SPEC §3「常駐なし」からの意図的変更**: SPEC §3 は取得バッチを「常駐なし(cron + ワンショット)」と定めているが、Docker 運用ではユーザー指示により cron 不要の scheduler 常駐へ意図的に変更した。非 Docker 運用(`make fetch` + cron)は従来どおり「常駐なし」のままで、[`scripts/crontab.example`](./scripts/crontab.example) はそのために残してある。

### 前提

- **ホストに Docker(Docker Engine + Compose v2)がインストールされていること。** これらのコマンドはホスト側で実行する(dev コンテナ内には docker CLI は無い)。
- `.env` を用意しておく(未作成なら `cp .env.example .env` して `FRED_API_KEY` を設定)。fetcher / scheduler はこの `.env` を `env_file` として読み込む(イメージには焼き込まない)。**scheduler が常駐化した都合で `make docker-up` も `.env` が無いと起動に失敗する**ので、先に用意すること。
- 構成: **web** は `data/`(SQLite)と `config/regime.yaml` を読み取り専用でマウントして表示するだけ。`data/` に書き込む(UPSERT)のは **scheduler / fetcher** のみ。DB パスは各サービスとも環境変数 `MACRO_DB_PATH=/app/data/macro.sqlite`、タイムゾーンは `TZ=Asia/Tokyo`。

### 手順

```sh
make docker-build   # イメージをビルド(web + fetcher/scheduler 共有イメージ)
make docker-up      # web + scheduler をバックグラウンド起動
# ブラウザで http://localhost:37414 を開く
make docker-down    # 停止・後片付け

make docker-fetch   # (任意)今すぐ1回取得したいときの手動ワンショット
```

scheduler は起動直後にも1回取得する(`FETCH_ON_START=1` が既定。コンテナ再起動時に即データが揃う)ため、初回も `make docker-up` だけでよい。ただし起動時取得は時刻を選ばず走るため、**IB を一次ソースにする環境では `FETCH_ON_START=0` を推奨**(詳細は後述「定期取得」の「起動直後の取得」の注意を参照)。
...
**所有権の注意(Linuxホスト):** scheduler / fetcher コンテナは root で動くため、取得が新規作成するファイル(初回の `data/macro.sqlite` 等)はホスト側で root 所有になる。その後ホスト側で非 Docker の `make fetch` / `make init` を併用する場合は、先に `sudo chown -R $USER data/` で所有権を戻すこと(Docker だけで運用する場合は不要)。

### 定期取得(scheduler。cron 不要)

`make docker-up` で常駐する scheduler が、毎日 `FETCH_AT`(既定 **06:30** JST、コンテナは `TZ=Asia/Tokyo`)に全系列を取得する。**Docker 運用では cron の設定は不要**([`scripts/crontab.example`](./scripts/crontab.example) は非 Docker 運用向けとして残してある)。

- **時刻の変更**: 起動時に環境変数 `FETCH_AT`(`HH:MM` 形式)を渡す。シェルから `FETCH_AT=05:15 make docker-up`、または `docker/.env` に `FETCH_AT=05:15` と書いてもよい(Compose が変数展開に読む)。**時刻は取引システムの稼働に影響しない時間帯(例: 早朝メンテ時間帯)を選ぶこと(SPEC §4.4)。** IB を一次ソースにしている場合、取得はワンショット接続とはいえ取引セッション中の実行は避ける。
- **起動直後の取得**: `FETCH_ON_START`(既定 `1`)が真なら scheduler 起動直後にも1回取得する。`FETCH_ON_START=0` で無効化。**注意(SPEC §4.4)**: この起動時取得は `FETCH_AT` と違って時刻を選ばない。`restart: unless-stopped` のため、ホスト再起動・Docker デーモン再起動・クラッシュ復帰・取引セッション中の `make docker-up` でも即座に実行され、IB を一次ソースにしていればワンショット接続が取引セッション中に走り得る。**IB を一次ソースにする環境では `FETCH_ON_START=0` を推奨**(`docker/.env` に書くか `FETCH_ON_START=0 make docker-up`)。初回データはメンテ時間帯に `make docker-fetch` で入れればよい。
- **失敗耐性**: 取得の失敗(例外・全系列失敗)で scheduler は死なず、エラーをログして次のスケジュールへ継続する。ログは `docker compose -f docker/docker-compose.yml logs -f scheduler` で確認できる(各実行の前後に次回実行時刻が出る)。
- 取得は冪等(UPSERT)なので、scheduler と手動の `make docker-fetch` が重なっても DB は壊れない。

### セキュリティ: 直接IPアクセスの制限(Cloudflare Tunnel 等で公開する場合)

Cloudflare Tunnel(`cloudflared`)のようなリバースプロキシで外部公開する場合、Docker 自身がホストの全インターフェース(`0.0.0.0`)にポートを開く必要は無い。`cloudflared` は同一ホストから **localhost 経由でアクセスできれば足りる**(cloudflared 側が外向きに接続するため、ポートの inbound 公開は不要)。

このリポジトリの `docker/docker-compose.yml` は web の `ports` を **`127.0.0.1:${MACRO_WEB_PORT:-37414}:3000`** に限定してある。これによりホストのグローバル IP からポート番号を直接叩いてもコンテナには届かず、`cloudflared` の宛先(`http://localhost:37414` 等)には影響しない。`cloudflared` を別コンテナで動かす場合は同一 Docker ネットワークに参加させ `http://web:3000` を宛先にすること(この場合は `127.0.0.1` バインドのままでよい)。

**注意: Docker は iptables を直接操作するため、UFW 等ホスト側ファイアウォールの deny ルールは Docker が公開したポートを素通りすることがある**(Docker が `DOCKER-USER` チェーンに転送ルールを追加し、UFW の通常チェーンより先に評価されるため)。上記のループバック限定バインドはこの問題自体を回避する(そもそもポートが公開されないため UFW の可否に依存しない)。UFW 側でも多層防御したい場合は次のいずれかを行う:

```sh
# 方法A: ufw-docker(推奨・コミュニティメンテのUFW/Docker統合スクリプト)
sudo wget -O /usr/local/bin/ufw-docker https://github.com/chaifeng/ufw-docker/raw/master/ufw-docker
sudo chmod +x /usr/local/bin/ufw-docker
sudo ufw-docker install
sudo systemctl restart ufw
sudo ufw status verbose   # DOCKER-USER 相当のルールが入っていることを確認

# 方法B: DOCKER-USER チェーンへ直接ルールを追加(WANインターフェースを eth0 と仮定)
sudo iptables -I DOCKER-USER -i eth0 -p tcp --dport 37414 -j DROP
```

方法Bは再起動で消えるため、恒久化するなら `iptables-persistent` を導入するか `/etc/ufw/after.rules` に追記すること。**127.0.0.1 バインド済みの現状では上記は "念のための多層防御" であり、必須ではない** — 将来 `docker-compose.yml` を書き換えて `0.0.0.0` 公開に戻した場合の保険として機能する。
