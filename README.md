...
現在の実装状況: **Phase 1(MVP)完了**。FRED / Yahoo Finance から §4.1 の全21系列を取得し、Next.js ダッシュボード(20パネル+レジーム・ストリップ静的版)で表示する。
...
| 13 | `YF:GC=F` | Yahoo | GC=F | 金先物 | USD | D |
| 14 | `YF:CL=F` | Yahoo | CL=F | WTI原油 | USD | D |
| 15 | `YF:HG=F` | Yahoo | HG=F | 銅先物 | USD | D |
| 16 | `YF:NG=F` | Yahoo | NG=F | 天然ガス | USD | D |
...
| 21 | `YF:JPY=X` | Yahoo | JPY=X | USD/JPY(補助) | 円 | D |

頻度は `D`(日次)/ `W`(週次)/ `M`(月次)/ `A`(年次)。
...
- **`YF:JPY=X` は補助系列。** USD/JPY の正は FRED の `DEXJPUS`(#7)。yfinance の `JPY=X`(#21)は日次の補助・クロスチェック用として併設する(SPEC §4.1 通貨欄の「補助として yfinance `JPY=X`」)。
